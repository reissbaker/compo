!function(exports) {
  ...
  function nodeMethod(fn) {
    return function(delta, x, y, z) {
      fireNext(this.components);
      each4args(this.components, fn, delta, x, y, z);
      fireEnd(this.components);
      fireNext(this._children);
      each4args(this._children, fn, delta, x, y, z);
      fireEnd(this._children);
    };
  }
  ...
  function fireEnd(cs) {
    for(var index = 0, length = cs.length; index < length; index++) {
      cs[index].end.fire();
    }
  }

  function fireNext(cs) {
    for(var index = 0, length = cs.length; index < length; index++) {
      cs[index].next.fire();
    }
  }
