!function(exports) {
  ...
  SceneNode.prototype._init = function() {
    this._initialized = true;
    this.init();
    each(this.components, fireInit);
    each(this._children, firePrivateInit);
  };

  SceneNode.prototype._destroy = function() {
    this.destroy();
    each(this.components, fireDestroy);
    each(this._children, firePrivateDestroy);
  };
  ...
  SceneNode.prototype.before = function(delta, x, y, z) {
    ...
    runMethods(this, method('before'), delta, this.x + x, this.y + y, this.z + z);
  };
  ...
  SceneNode.prototype.after = function(delta, x, y, z) {
    ...
    runMethods(this, method('after'), delta, this.x + x, this.y + y, this.z + z);
  ...
  SceneNode.prototype.add = function(child) {
    if(child) {
      ...
      child._parent = this;
      if(this._initialized && !this._destroyed) child.init();
    }
  };
  ...
  SceneNode.prototype.remove = function(child) {
    for(var curr, index = 0; curr = this._children[index]; index++) {
      if(curr === child) {
        ...
        curr._parent = null;
        if(this._initialized && !this._destroyed) curr.destroy();
        return true;
      }
    }
    return false;
  ...
  function nodeMethod(fn) {
    return function(delta, x, y, z) { 
      runMethods(this, fn, delta, this.x + x, this.y + y, this.z + z);
    };
  }
  ...
  function fireInit(c) { c.init(); }
  function fireDestroy(c) { c.destroy(); }
  function firePrivateInit(c) { c._init(); }
  function firePrivateDestroy(c) { c._destroy(); }
